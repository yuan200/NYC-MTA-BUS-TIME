...
public class SearchResultActivity extends AppCompatActivity {
    ...
    private ArrayList<StopMonitoringListItem> mSaveForFavoriteList;
    ...
    public class FetchBusTimeTask extends AsyncTask<URL, Void, ArrayList<StopMonitoringListItem>> {
        @Override
        protected ArrayList<StopMonitoringListItem> doInBackground(URL... params) {
            URL searchUrl = params[0];
            String busTimeInqueryResult = null;
            StopMonitoringListItem expectedTime = null;
            ArrayList<StopMonitoringListItem> stopsStopMonitoringListItem = null;
            try {
                ...
                stopsStopMonitoringListItem = (ArrayList<StopMonitoringListItem>) NetworkUtilities.getSpecificItem(busTimeInqueryResult, "ExpectedArrivalTime");
            } catch (IOException e) {
                e.printStackTrace();
            }
            //return expectedTime;
        ...
        @Override
        protected void onPostExecute(ArrayList<StopMonitoringListItem> stopsStopMonitoringListItem) {
            Date strToDate;
            SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ");
            ArrayList<String> busLineName = new ArrayList<>();
            StopMonitoringListItem errorChecking;
            StopMonitoringListItem expectedTime;
            StopMonitoringListItem saveForFavorite;
            ...
            if (stopsStopMonitoringListItem == null) {
                timeInfoView.setText("Sorry, something went wrong, please try again later");
                mSwipeRefreshLayout.setRefreshing(false);
                return;
            }
            expectedTime = stopsStopMonitoringListItem.get(0);
            errorChecking = stopsStopMonitoringListItem.get(0);
            ...
            String boldText = stopsStopMonitoringListItem.get(0).getStopPointName();
            saveForFavorite = new StopMonitoringListItem();
            saveForFavorite.setStopNumber(stopsStopMonitoringListItem.get(0).getStopNumber());
            saveForFavorite.setStopPointName(stopsStopMonitoringListItem.get(0).getStopPointName());
            ...
            for (int i = 0; i < stopsStopMonitoringListItem.size(); i++) {
                 ...
                 **/
                expectedTime = stopsStopMonitoringListItem.get(i);
                isSameBusLine = false;
                ...
                timeInfoView.append(expectedTime.getPublishedLineName() + " To " + expectedTime.getDestinationName() + "\n");
                ...
                if (!expectedTime.getExpectedDepartureTime().equals("NoOriginAimedDepartureTime")) {
                    Log.i("timedebug====", "OriginAimDepartureTime: " + expectedTime.getExpectedDepartureTime());
                    try {
                        strToDate = format.parse(expectedTime.getExpectedDepartureTime());
                        Calendar cal = Calendar.getInstance();
                        cal.setTime(strToDate);
    ...
    public void addToFavorite() {
        ...
        String groupName = "";
        for (StopMonitoringListItem item : mSaveForFavoriteList) {
            ...
            groupName = item.getStopPointName();
        }
        //todo disable this button if no info
        if (stopCode != null) {
            FavoriteStop favorite = new FavoriteStop(stopCode, "", "", "", "",
                    groupName, new Date());
    ...
    public void removeFromFavorite() {
        int id = (int)mRowID;
        Executors.newSingleThreadExecutor().execute(() ->
                BusDatabase.Companion.getInstance(getApplicationContext()).favoriteStopDao().delete(id));
    }
